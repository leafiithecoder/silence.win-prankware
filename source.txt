...
namespace silence.win
{
    public class Class1
    {
        ...
        static extern bool TextOut(IntPtr hdc, int nXStart, int nYStart, string lpString, int cCount);
        ...
        public static void Main()
        {
            ...
            SoundPlayer player = new SoundPlayer(Properties.Resources.track); // fire.wav added as resource

            DialogResult result = MessageBox.Show(
                "Are you sure? This will break your PC beyond repair.",
                "Silence",
            ...
            {
                MessageBox.Show("Goodbye.", "Silence", MessageBoxButtons.OK, MessageBoxIcon.Error);
                player.PlayLooping(); // loops until you call Stop()
            }
            else
            {
                MessageBox.Show("Good choice.", "Silence", MessageBoxButtons.OK, MessageBoxIcon.Information);
                Environment.Exit(0);
            }
            ...
            while (true)
            {
                ...
                foreach (var screen in Screen.AllScreens)
                {
                    ...
                    int offsetY = screen.Bounds.Y;

                    IntPtr hdc = GetDC(IntPtr.Zero);
                    IntPtr hIcon = LoadIcon(IntPtr.Zero, new IntPtr(32513)); // IDI_ERROR
                                                                             // Draw the "YOUR PC IS ON DRUGS" text in red
                                                                             // Draw the text at a random position (spawns like the icon)
                    string message = "YOUR PC IS ON DRUGS";
                    TextOut(hdc, r.Next(width - 200), r.Next(height - 50), message, message.Length);


                    int x = width / 4;   // roughly center horizontally
                    int y = height / 2;  // roughly center vertically
                    TextOut(hdc, x, y, message, message.Length);

                    DrawIcon(hdc, r.Next(Screen.PrimaryScreen.Bounds.Width), r.Next(Screen.PrimaryScreen.Bounds.Height), hIcon);
                    IntPtr Brush = CreateSolidBrush(rndclr[r.Next(rndclr.Length)]);
                    SelectObject(hdc, Brush);
                    int rand = r.Next(height);
                    PatBlt(hdc, offsetX, offsetY, width, height, TernaryRasterOperations.PATINVERT);
                    BitBlt(hdc, rand, r.Next(-4, 4), r.Next(100), height, hdc, rand, 0, TernaryRasterOperations.SRCCOPY);
                    DeleteObject(Brush);
                    DeleteDC(hdc);
                    Thread.Sleep(15);
                }
